...
WORDLIST_FILENAME = "C:/Users/praty/OneDrive/Desktop/6000x_problemset/ProblemSet4/words.txt"
...
def getWordScore(word, n):
    ...
    s_letters=0
    for l in word:
        s_letters+=SCRABBLE_LETTER_VALUES[l]
    if len(word)==n:
        score= s_letters*len(word)+50
        return score
    else:
        score= s_letters*len(word)
        return score
...
def updateHand(hand, word):
    ...
    result=hand.copy()
    for l in word:
        assert result.get(l,0)>0, "Not enough "+l+" in hand."
        if l in result:
            result[l]-=1
    return result
